...
- **migemo 対応のあいまい検索** — [cmigemo](https://github.com/koron/cmigemo) を FFI で呼び出し、ローマ字のまま日本語フォルダ名・ファイル名を検索できる（例: `kensaku` → 「検索」にヒット）。
...
| 文字入力 | migemo であいまい一致した結果をリアルタイム表示（最大20件） |
| `↑` / `↓` | 候補選択を移動 |
