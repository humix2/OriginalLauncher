...
- **インストール済み Steam ゲームの検索・起動** — レジストリと Steam のライブラリメタデータ（`libraryfolders.vdf` / `appmanifest_*.acf`）からインストール済みタイトルを検出し、通常のファイル/フォルダと同じ一覧・検索で `steam://rungameid/<appid>` 経由で起動できる。アイコンは Steam のライブラリキャッシュ画像を利用。
